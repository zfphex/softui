...
#[thread_local]
pub static mut ARENA: LazyLock<Arena<Node>> = LazyLock::new(|| Arena::new(1024 * 1024));

unsafe fn alloc(node: Node) {
    unsafe {
        ARENA.alloc(node);
    }
}
...
fn main() {
    unsafe { alloc(Node::default()) };
    // unsafe { for node in ARENA.iter() {} }
}
