...
mod tests {
    ...
    fn test_many_fill_children_precision() {
        ...
        assert!((last_end - 1000.0).abs() < 0.01, "Accumulated positions should equal parent size");
    }
    ...
    fn test_single_percentage_child() {
        ...
        assert!((node.size[0] - 240.0).abs() < 0.01, "{}: width {} != 240.0", node.name, node.size[0]);
        assert!((node.size[1] - 120.0).abs() < 0.01, "{}: height {} != 120.0", node.name, node.size[1]);
        check_pos(&tree, c, 0.0, 0.0);
    }
